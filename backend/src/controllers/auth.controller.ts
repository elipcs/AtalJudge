...
import { validateBody, authenticate, AuthRequest, convertUserRegisterPayload, authRateLimiter, registerRateLimiter, requireProfessor } from '../middlewares';
...
function createAuthController(
  ...
  generateResetLinkUseCase: GenerateResetLinkUseCase
): Router {
  const router = Router();
  ...
  router.post(
    '/professor/generate-reset-link/:userId',
    authenticate,
    requireProfessor,
    asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
      const { userId } = req.params;
      const result = await generateResetLinkUseCase.execute(userId);
      successResponse(res, result, 'Reset link generated successfully');
    })
  );

...
function createQuestionListController(
  ...
  getQuestionListMetricsUseCase: GetQuestionListMetricsUseCase
): Router {
  const router = Router();
  ...
  router.get(
    ...
    requireTeacher,
    asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
      const metrics = await getQuestionListMetricsUseCase.execute(req.params.id);

      successResponse(res, metrics, 'Question list metrics found');
    })
  );
